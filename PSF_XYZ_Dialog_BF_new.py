...
class PSF_XYZ_Dialog_BF(QDialog, ui_PSF_XYZ_Dialog_BF_new.Ui_PSF_XYZ_Dialog_BF_new):
    ...
    def display_results(self, xdim, ydim, zdim, stack, imagefilepath, planexy, xpos, ypos, zpos,
        ...
        # only save plot when option is checked
        if self.checkBox_SaveOrthoView.isChecked():
    ...
    def onstart_detection(self):
        ...
        #imagefilepath = str(self.text_filename.text())
        imagefile = os.path.basename(self.imagefilepath)
        imagedir = os.path.dirname(self.imagefilepath)

        # fill BeadData
        self.BeadData['FilePath'] = self.imagefilepath
        ...
        imagestack, dimorder_out = bf.get_zstack(self.imagefilepath,
                                                 self.MetaInfo['Sizes'],
                                                 seriesID,
                                                 timepoints='single',
                                                 tindex=0)
        imagestack.astype(int)
        imagestack = np.squeeze(imagestack[:, channel, :, :])
...
def cut_subimages(peaknumber, peakx, peaky, zplanes, subimagesize, stack):
    ...
    sz = int(np.round(subimagesize/2, 0) + 1)
...
def calc_average_psf(imagestack, xpos, ypos, zdim, psfwidth):
    ...
    avgPSF = np.zeros([zdim, 2 * int(psfwidth), 2 * int(psfwidth)])
    zdim = int(zdim)
    psfwidth = int(psfwidth)
