...
class PSF_XYZ_Dialog_BF(QDialog, ui_PSF_XYZ_Dialog_BF.Ui_PSF_XYZ_Dialog_BF):
    ...
    def find_peaks(self, planexy, zpos):
        ...
        peaks = sf.peak_local_max(planexy, min_distance=mindist, threshold_rel=th, exclude_border=True, indices=True, num_peaks=inf)
    ...
    def display_results(self, xdim, ydim, zdim, stack, imagefilepath, planexy, xpos, ypos, zpos,
        ...
        # ax2
        # ax2.imshow(img, interpolation='nearest', origin=None,  cmap=cm.jet)
        ax2.imshow(igl[img2show], interpolation='nearest', origin=None,  cmap=cm.jet)
        ...
        # PSFstack = stack[zpos-dz:zpos+dz, psfy-psfwidth:psfy+psfwidth, psfx-psfwidth:psfx+psfwidth]
        PSFstack_avg = calc_average_psf(stack, xpos, ypos, zdim, psfwidth)
        # psf.psf_orthoview(PSFstack, psfwidth, dz, ratio)
        # psf.psf_volume(PSFstack, ratio)
    ...
    def onstart_detection(self):
        ...
        # PeakFind algorithm from ImageP for pre-detection of beads
        [xpos, ypos, peaknum] = self.find_peaks(planexy, zpos)
...
def calc_average_psf(imagestack, xpos, ypos, zdim, psfwidth):
    ...
    # calc the mean
    avgPSF = np.round(avgPSF/len(xpos), 0)
...
def estimate_fwhmz(na=1.0, wl=0.515, imm='Air'):
    # this is JUST an estimation how to come up with a goof guess for the FWHM-Z value
